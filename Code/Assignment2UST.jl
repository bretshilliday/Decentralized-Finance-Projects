...
function calculateBalances(df::DataFrame, wallets::Dict)
    for dfRow in eachrow(df)
        if !haskey(wallets, dfRow.topic1)
            wallets[dfRow.topic1] = [BigInt(0),BigInt(0),BigInt(0)]
        end
        if !haskey(wallets, dfRow.topic2)
            wallets[dfRow.topic2] = [BigInt(0),BigInt(0),BigInt(0)]
        end
...
function prepareDataframe(df::DataFrame)
    ...
    df.amount_sent = parse.(BigInt, df.data0, base=16) # Converts the amount sent in data0 to be a decimal number rather than a Hexidecimal String
...
function main(inputFile::String)
    ...
    dailyMetrics = DataFrame(day = String[], numberOfTransactions = BigInt[], volumeOfTransactions  = BigInt[])
    for sdf in gdf
        day = first(sdf).day
    ...
    CSV.write("/home/bret.shilliday/FNCE559.09/Homework/Assignment2/FNCE55909A2/CSV/USTWalletInformation.csv", walletsFrame)
    CSV.write("/home/bret.shilliday/FNCE559.09/Homework/Assignment2/FNCE55909A2/CSV/DailyUSTInformation.csv", dailyMetrics)
end

# -- Begin Executable -- 

# -- Variables --
inputFile = "/home/DefiClass2022/databases/t_token_A47C8BF37F92ABED4A126BDA807A7B7498661ACD.jld2"
